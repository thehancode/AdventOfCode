...
function generateMissing(levels: number[]): number[][] {
  const result: number[][] = [];

  for (let i = 0; i < levels.length; i++) {
    const copy = levels.slice(0, i).concat(levels.slice(i + 1));
    result.push(copy);
  }
...
const recheckLevels = (levels: number[]) => {
  ...
  const missing = generateMissing(levels)

  console.log("missing :", missing)
  for (let i = 0; i < missing.length; i++) {
    if (checkIfSafe(missing[i]) && checkIfOrdered(missing[i])) {

      console.log("didit : ", missing[i])
      return true
    }
  }
  return false
